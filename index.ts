...
class CX {
    ...
    constructor(username: string, password: string, courseName: string[]) {
    ...
    async getAllCourses() {
        ...
        if (courses.data.result === 1) {
            ...
            this.filterCourse = this.courses.filter(item => {
                return item.content.course.data.some((items: { name: string; }) => this.courseName.includes(items.name))
            })
    ...
    async pass_video({
        ...
        while (true) {
                ...
                playingTime += 60 * this.speed
                b1.update(playingTime)
                await sleep(1000*60) // 每次播放间隔2秒
            }
    }
    ...
    async get_workInfo({
        ...
        await this.acquisitionProblem(data.data)
    }
        ...
        for (let course of this.filterCourse) {
            ...
            for (let item of mission) {
                ...
                for (let cardIndex in knowledge_raw[0].card.data) {
                    ...
                    // 匹配章节信息
                    const attachments = JSON.parse((String(data).match('\window.AttachmentSetting =({\"attachments\":.*})') as any[])[1])
                    if (!attachments) {
                        continue
                    }
...
(async function () {
    const cx = new CX("账号", '密码', ['课程名'])
    await cx.login()
    await cx.getAllCourses()
    await cx.doWork()
